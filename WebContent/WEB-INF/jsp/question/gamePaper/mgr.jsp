...
<body>
	<%
		...
		answerArr = new Integer[6];
		if(!StringUtils.isEmpty(answer)){
			String[] anArr = answer.split(",");
			for(int i=0;i<anArr.length;i++){
				int anValue = Integer.parseInt(anArr[i]);
				answerArr[anValue] = anValue;
			}
		}
			...
			<form class="form-horizontal ">
				...
				<ul class="page-breadcrumb breadcrumb">
					<div class="form-group">
						...
						<div class="col-sm-10">
						<input type="hidden" name="qbType" value='<%=qbType%>'>
							<p class='form-control-static'><%=qbTypeName%></p>
						</div>
					</div>
					<div class="form-group">
							...
							<div id="ctn_options">
								...
								<c:if test='${item!=null&&item ne ""}'>
											...
											<div class="input-group" style="margin-bottom: 10px;">
												...
												<input class='form-control option' name='option<%=optionIndex++%>' 
					...
					<div class="form-group">
						...
						<div class="col-sm-6" id='ctn_option_answer'>
						
						<c:forEach items='<%=optionArr%>' var="item">
								<c:if test='${item!=null&&item ne ""}'>
		
											<label class='checkbox-inline' id='answer<%=optionAnswerIndex%>'>
												  <input type='checkbox' class='answer' name='answer' value='<%=optionValue++%>' <%=answerArr[optionAnswerIndex]!=null?"checked":"" %>> <span><%=prefixArr[optionAnswerIndex++]%></span>
												</label>
...
<script type="text/javascript">
	$(function() {
		...
		$("#btn_add_option").click(function(e){
			...
			var optionAnswerTemplate = $("<label class='checkbox-inline' id='answer"+optionCount+"'><input type='checkbox' class='answer' name='answer' value='"+optionCount+"'> <span>"+prefixArr[optionCount]+"</span></label>");
			$("#ctn_option_answer").append(optionAnswerTemplate);
		...
		function del_option(_this){
			...
			var i_name = $(_this).parent().parent().children('input').attr('name');
			var index = i_name.substring(6);
			$('#ctn_option_answer').children("#answer"+index).remove();
			optionCount--;
			...
			$.each($('#ctn_option_answer label'),function(i,v){
				$(v).children().text(prefixArr[i]);
				$(v).attr('id','answer'+i);
				$(v).children('input').val(i);
			});
		}
			...
			submitHandler:function(form){
				...
				$.ajax({
					...
					success : function(data) {
						alert("操作成功!");
						window.location.href = "/ejwms/question/gamePaper2";
					},
					error : function() {
						alert('系统出现异常，请稍后再试!');
					}
