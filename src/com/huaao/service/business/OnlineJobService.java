...
public class OnlineJobService {
	...
	*/
	...
	public void queryForPage(Integer communityid, Page pager) {
		StringBuffer sbf = new StringBuffer("SELECT a.id apply_id, c.name pupil_community, a.keeperid apply_keeperid, a.status audit_status, a.description, a.hospital, a.createtime,cuser, b.* FROM sun_ejingwu.sps_d_cure_apply a left join sps_b_pupil_info b on a.pupilid = b.id left join jw_community c on b.communityid = c.id where a.id is not null and b.id is not null and a.communityid = " + communityid);
