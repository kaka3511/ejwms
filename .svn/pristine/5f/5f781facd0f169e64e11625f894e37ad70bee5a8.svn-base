...
<script type="text/javascript">
	$(function() {
		var currentQuestionGamePage = getOneSessionStorageValue("currentQuestionGamePage");
		$(window).resize(function(){
			$("#jqGrid").setGridHeight($(window).height()-$('.panel-body').offset().top-80);
		});
		$("#jqGrid").jqGrid({
			//url : "<c:url value='/question/list?qbType=1'/>",
			url : "<c:url value='/question/list1'/>",
			// we set the changes to be made at client side using predefined word clientArray
			editurl : 'clientArray',
			datatype : "json",
			mtype: 'POST',
			page : currentQuestionGamePage,
			jsonReader : {
				root : 'data.rows',
		...
		function operFormat(cellvalue, options, rowObject) {
			var currentQuestionGamePage = $(this).jqGrid('getGridParam','page');
			sessionStorage.setItem("currentQuestionGamePage", currentQuestionGamePage); 
		...
		function operUnFormat(cellvalue, options, cell) {
	     ...
	     $('#query_type').change(function(){
	    	var self = this;
	     	...
	     	var query_type12 = value;
			setCookie("query_type12", query_type12, 0.1);
...
if(!(!!window.ActiveXObject || "ActiveXObject" in window))		 	
	 	{
	 			var query_type12 = getCookie('query_type12');

	 			if (query_type12 == "1" || query_type12 == "2"
					|| query_type12 == "3")
				$('#query_type').val(query_type12);
	 		}	     
