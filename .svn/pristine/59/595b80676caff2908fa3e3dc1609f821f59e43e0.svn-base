...
<!-- BEGIN BODY -->
<body>	
	<input type="hidden" name="communityLocation" id="communityLocation" value="${community.location}">
	<input type="hidden" name="communityArea" id="communityArea" value="${community.area}">
	...
	<div class="panel panel-info">
		<div class="panel-heading">
			社区基本资料
			<button class="btn-success pull-right updateInfo">修改</button>
			<button class="btn-info pull-right cancelInfo showInfo hidden">取消</button>
			<div class="pull-right">&nbsp;&nbsp;&nbsp;</div>
			<button class="btn-primary pull-right saveInfo showInfo hidden">保存</button>
		</div>
		<div class="panel-body">
			<div class="row">
				<div class="col-sm-4">
					<label class="col-sm-3 control-label">社区名称：</label>
					<div class="col-sm-9"><input type="text" class="form-control col-sm-6 disableInfo" placeholder="社区名称不能超过64个字符" id="communityName" name="communityName" disabled="disabled" value="${community.name}"></div>
				</div>
				<div class="col-sm-4">
					<label class="col-sm-3 control-label">管理密码：</label>
					<div class="col-sm-9"><input type="password" class="form-control col-sm-6 disableInfo" placeholder="社区名称不能超过32个字符" id="communityPW" name="communityPW" disabled="disabled" value="${community.passwd}"></div>
				</div>
				<div class="col-sm-4">
					<label class="col-sm-3 control-label">社区地址：</label>
					<div class="col-sm-9"><input type="text" class="form-control col-sm-6 disableInfo" placeholder="社区地址不能超过64个字符" id="communityAddress" name="communityAddress" disabled="disabled" value="${community.address}"></div>
				</div>
			</div>
			<div class="row">
				<div class="col-sm-4">
					...
					<div class="col-sm-9">
						<textarea class="form-control col-sm-1 disableInfo" rows="4" placeholder="社区描述不能超过64个字符" id="communityDescription" name="communityDescription" disabled="disabled">${community.description}</textarea>
					</div>
				</div>
				<div class="col-sm-4">
					<label class="col-sm-3 control-label">社区图片：</label>
					<div class="col-sm-9">
						<img alt="" src="${community.img}" width=200 height=100 id="communityImg">
					</div>
				</div>
				<div class="col-sm-4 hidden imgDiv">
		...
		<div class="panel-heading">
			社区地图定位
			<button class="btn-success pull-right updateLocation setLocation" id="updateLocation">修改社区位置</button>
			<div class="pull-right">&nbsp;&nbsp;&nbsp;</div>
			<button class="btn-success pull-right updateArea setLocation" id="updateArea">修改社区区域</button>
...
var communityLocation = $('#communityLocation').val();
var communityArea=$("#communityArea").val();
    ...
    AMap.event.addDomListener(document.getElementById('saveLocation'),'click', function () {   //绑定保存按钮
    	if(marker==null){
    		...
    		$.ajax( {
    			...
    			async:false,   //设置为同步
    			url : "/ejwms/communityManage/saveCommunityLocation.do",//请求的uri
    			data : {location:marker.getPosition().toString()},//传递到后台的参数				
    			cache : false,
...
$(document).on('click','.updateInfo',function(){
	...
	$('.disableInfo').attr("disabled",false);
...
$(document).on('click','.saveInfo',function(){
	...
	var password = "";
	if(communityPW!=$("#communityPW").val()){  //没有修改密码
		password = $("#communityPW").val();
	}
	$.ajaxFileUpload({  
        url : '/ejwms/communityManage/saveCommunityInfo.do',  
