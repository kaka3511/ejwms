...
<input type="hidden" name="parentId" id="parentId" value="${parentId}">
	...
	<div class="panel panel-info">
		<div class="panel-heading">
			  父类名称：<label id="parentName"></label> 
			<button class="btn btn-success pull-right addEntityBtn">新增</button>
  			<button class="btn btn-success pull-right gobackBtn">返回</button>
		</div>
			...
			<table class="table table-bordered table-striped table-hover entity_table">
				<thead>
					<tr>
						...
						<th class="text-center" style="background: #3499db; color: #ffffff; width: 20%;">显示名称</th>
						<th class="text-center" style="background: #3499db; color: #ffffff; width: 10%;">内部编码</th>
						<th class="text-center" style="background: #3499db; color: #ffffff; width: 10%;">代码值</th>
						<th class="text-center" style="background: #3499db; color: #ffffff; width: 10%;">排序</th>
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					<form class="form-horizontal entityform" role="form">
						...
						<div class="form-group">
							<label class="col-sm-2 control-label">字类图标</label>
							<div class="col-sm-6">
						...
						<div class="form-group">
							<label class="col-sm-2 control-label">内部编码</label>
						...
						<div class="form-group">
							...
							<div class="col-sm-6"><input type="text" class="form-control col-sm-6" id="value" name="value"></div>
						</div>
						<div class="form-group">
							<label class="col-sm-2 control-label">字类排序</label>
							<div class="col-sm-6"><input type="text" class="form-control col-sm-6" id="order" name="order"></div>
						</div>
						<div class="form-group">
							<label class="col-sm-2 control-label">字段状态</label> 
							<div class="col-sm-6">
	...
	$(".entity_table").dataTable({
	    ...
	    columns: [//对应上面thead里面的序列
	              ...
	              { data: function (e) {//这里给最后一列返回一个操作列表
	                  ...
	                  var sContent  = '<button class="btn btn-primary modify">修改</button>&nbsp;&nbsp;&nbsp;<button class="btn btn-primary delete">删除</button>';
...
$(document).on('click','.addEntityBtn',function(){
	...
	$('#id').val("");
	$('#type').val("0");
	$('#entityLabel').html("小类维护");
	$('#entityModal').modal('show');
	$('#communityImg').addClass("hidden");
});

$(document).on('click','.gobackBtn',function(){
	window.location.href="/ejwms/dictionaryManage/dictionary";
});
				...
				secureuri : false,//安全协议  
		 	    ...
		 	    fileElementId:'uploadImg',
	      		data : {
	      			...
	      			status:$("#status").val(),
	      			order:$("#order").val(),
	      			parentId:$("#parentId").val()},//传递到后台的参数				
